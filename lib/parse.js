...
class Parser {
  ...
  parse(program) {
    ...
    // Result tree
    const tree = (this.lookahead !== null) ? this.parseExpression() : null;
  ...
  parseExpression() {
    ...
    if(this.lookahead.type === 'STRING') {
      ...
      expr = {type: 'regex', body: fields[1], flags: fields[2] };
...
Parser.TOKENS = [tokenRegex.NUMBER, tokenRegex.STRING, tokenRegex.REGEX,
