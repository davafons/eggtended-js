    ...
    case 'apply':
      ...
      if(typeof op === 'object' || typeof op === 'string' || typeof op === 'number') {
        ...
        // If the name of the method is not defined on the object...
          ...
          if(typeof op['missing'] === 'function') {
            return op['missing'](...evArgs.slice(1));
  ...
  for (let scope = env; scope; scope = Object.getPrototypeOf(scope)) {
    if (Object.prototype.hasOwnProperty.call(scope, valName)) {
      ...
      if(indices.length === 0) {
        ...
        let lastArray = TopEnv.element(scope[valName], ...indices.slice(0,
          indices.length - 1));

        // Modify the value on the last array
        TopEnv.setelem(lastArray, indices[indices.length - 1], value);
      }
      return value;
    }
  }
...
SpecialForms['object'] = (args, env) => {
  ...
  const evArgs = args.map((arg) => SpecialForms.evaluate(arg, objectEnv));

  for(const pair of TopEnv._chunk(evArgs, 2)) {
    const name = pair[0];
    const value = pair[1];
...
TopEnv['map'] = TopEnv['dict'] = (...args) => {
  return new Map(TopEnv._chunk(args, 2));
}
...
TopEnv['<-'] = TopEnv['[]'] = TopEnv['element'] = (array, ...indices) => {
  ...
  indices.forEach((index) => {
    result = TopEnv.getelem(result, index);
  });

  return result;
};

TopEnv['getelem'] = (data, index) => {
  if (data instanceof Array) {
    index = TopEnv._getValidIndexFrom(data.length, index);
    return data[index];
    ...
    throw new TypeError(`Element in index (${index}) of object ${data} is not indexable`);
  }
}

TopEnv['setelem'] = (data, index, value) => {
  if (data instanceof Array) {
    index = TopEnv._getValidIndexFrom(data.length, index);
    data[index] = value;
...
TopEnv._getValidIndexFrom = (length, index) => {
  if (index < 0) {
    index = length + index;
  }
...
TopEnv._chunk = function*(arr, step) {
