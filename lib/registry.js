  ...
  for (let scope = env; scope; scope = Object.getPrototypeOf(scope)) {
    if (Object.prototype.hasOwnProperty.call(scope, valName)) {
      if(indices.length == 0) {
        ...
        // Get a reference to the last array pointed by the index
        let array = TopEnv.element(scope[valName], ...indices.slice(0,
          indices.length - 1));

        // Modify the value on the last array
        let last_index = TopEnv._getValidIndexFrom(array, indices[indices.length - 1]);
...
TopEnv['<-'] = TopEnv['[]'] = TopEnv['element'] = (array, ...indices) => {
  ...
  for(let i = 0; i < indices.length; ++i) {
    let index = TopEnv._getValidIndexFrom(array, indices[i]);
